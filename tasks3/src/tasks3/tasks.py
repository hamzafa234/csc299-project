...
def add_task(description):
    ...
    # *** REFACTOR: Set 'low' priority directly in the task dictionary ***
    default_priority = "low"
    
    task = {
        "id": max_id + 1,
        "description": description,
        "priority": default_priority, # Use the default_priority variable
        "completed": False,
        "created_at": datetime.now().isoformat()
    }
    tasks.append(task)
    save_tasks(tasks)
    print(f"✓ Task added: {description} (Priority: {default_priority})")
    print(f"  Use 'change {task['id']}' to change priority")
...
def main():
    print("Running tasks3 main program...")
